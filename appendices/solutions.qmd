...
The comparison is `TRUE`: the two agree, including on where the `NA`s fall, which is the part to check: `identical()` treats `NA` as a value like any other, so a version that produced `NA` in a different place would come back `FALSE` even if every real number matched.
