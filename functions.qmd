...
Eighty-eight days. What happens at a stricter cut-off? Copy the block and change the number.
...
The heading says September; the number is December's. Nothing errors and nothing warns, and 12.3 is a believable figure for a quiet month, so there is nothing in the output to signal the mistake. A run from top to bottom therefore records December's figure under September's label.

This connects to the advice in the good practice chapter about not saving your workspace and restarting R often. In a fresh session, `month_data` does not exist, and the second calculation fails like this:
