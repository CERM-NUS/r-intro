...
Most quantities asked for in public health are not summaries of a whole dataset. They are summaries within subgroups: per month, per ward, per age band, per vaccination status. Answering a question such as how the monthly average moved across the circuit breaker means splitting the 345 rows into groups, computing something within each group, and combining the results back into a table.
...
The rule of thumb: use `summarise()` when the answer has fewer rows than the input, and a grouped `mutate()` when the answer has the same number of rows but each row needs a value computed from the rest of its group.
...
**character string is not in a standard unambiguous format**. It raises an error rather than returning `NA`, so a pipeline built on partial date strings stops rather than continuing with missing values. Rounding the date with `floor_date()` and leaving the column as a `Date` avoids the question.
...
Four months, April through July, which is the dormitory outbreak and the circuit-breaker period around it. August falls below the threshold with 4 High days despite containing the 908-case spike, because the rule counts days rather than cases and a single very large day is still one day. Whether that is the right rule depends on what the alert is for; the threshold and the quantity it counts are both choices made by whoever writes the rule.
