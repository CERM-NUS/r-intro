...
`non-numeric argument to binary operator` reports that the subtraction was attempted on text. This is why base-R instructions usually include a line such as `df$date <- as.Date(df$date)` near the top; with `read_csv()` the conversion has already happened.
