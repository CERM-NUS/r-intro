...
Writing `here("data", "sg_covid_data.csv")` costs a few extra characters and leaves the script working on any machine where the project has been cloned, whatever directory R was started from.
...
That block of text is a message rather than an error, and each line of it is worth reading.
...
All three differences are visible above: no column specification is printed, `class()` reports a plain `data.frame`, and `date` holds strings that look like dates. Arithmetic on that column fails:
...
No specification message this time, because there was nothing to guess. The quickest way to write one of these is to read the file once, run `spec()`, and paste the output into the script as the `col_types` argument. The import then either matches the stated specification or reports where it does not.
...
Row 2, column 1, expected a double, found `Singapore`. Two columns times 345 rows is 690 failures here. When a real import produces a handful of failures rather than hundreds, those few rows are usually the ones worth inspecting: typically a note typed into a numeric field.
...
It answers three questions at once. The dimensions confirm the import. The type in angle brackets after each name confirms the parse. The first few values show the content.
...
Nothing is cleaned and nothing is repaired. Counts are still cumulative and `NA`s are still `NA`s. Where a later chapter corrects something, it does so in code you can read.
