...
You should get 345, and above it a column specification listing 13 columns: one character, eleven numbers and one date. Read that block rather than scrolling past it. It is readr telling you what it decided each column was. Those decisions are guesses made from the first thousand rows, and the block is where you catch a bad one, which is a great deal cheaper than catching it four steps into an analysis when a date turns out to have been text all along.
...
Yours will print something else. Do not repair it by typing `setwd()` and a path. Take the folder you made for this course and give it a project file: File, New Project, Existing Directory, point RStudio at the folder. From then on you open your work by double-clicking the `.Rproj` file that appears, R starts in that folder every time, and `data/sg_covid_data.csv` resolves without you typing a path at all.
