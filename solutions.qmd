...
Seven elements, and the type is `double` rather than `integer` even though every value is a whole number. `c(0, 1, 3)` writes doubles unless you ask for integers with `0L, 1L, 3L`. The distinction is worth knowing when you check `typeof()` output against what you expected, and it almost never matters in practice.
...
Eighty-eight, the same answer as before the rewrite. Adding an argument in the middle of a signature breaks every existing call that relied on position, and putting the new argument after `df` with a default that reproduces the old behaviour is what keeps that from happening.
