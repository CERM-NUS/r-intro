...
Every exercise in the book has an answer here, and every answer on this page was executed to produce the output you see. If a number in a solution disagrees with a number you got, one of us has a bug, and the discrepancy is worth tracking down.
...
`non-numeric argument to binary operator` is one of the errors you will meet most often in your first month of R, and the usual cause is exactly this: a stray label, a footnote marker, or a `"N/A"` typed by hand into a spreadsheet column, coercing an entire column of numbers to text on import.
...
`NA`, not `FALSE`. `NA < 0` is `NA`, because R has no idea whether the missing value was negative. `any()` then has a vector containing `NA`s and `FALSE`s and no `TRUE` at all, and the honest answer to "is at least one of these true?" when some of them are unknown is "I cannot tell". So it returns `NA`.
...
## Move the cutoffs {#sec-sol-move-cutoffs}
...
## A monthly alert table {#sec-sol-monthly-alert}
...
The honest version of the answer is that I would report both columns and let the meeting see the disagreement. A single status column hides the fact that August is classified one way by one measure and the other way by another.
...
That is why `high_cut` is an argument rather than a hard-coded 5. Not because the caller will change it often, but because a default in the signature is visible to anyone reading the function, whereas the same number written into the body has to be hunted for. Set it once, at the top, where a reviewer can see it and disagree with it:
...
Five team sizes means five runs, and copying the loop four times with one number changed is the failure mode @sec-functions is about. `max(0, ...)` stays inside the loop, because moving it outside is what gives you the flat zero line the chapter showed.
...
Without 5 August the mean falls from 148.6 to 123.3 and the median barely moves, from 91 to 89. The median was already describing a typical August day, which is the reason to report one alongside the mean in a monthly table.
