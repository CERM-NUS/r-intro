...
Computing it takes one line of dplyr. Most of this chapter is about the assumptions that line makes.
...
dplyr is built on a small number of functions with deliberately plain names. Each one takes a data frame as its first argument, does one thing to it, and returns a data frame. They are called verbs because they are things you do to a table.

Five of them cover most routine work.
...
Fifty-six rows, which is 7 April to 1 June counted inclusively. Two things in that call are worth noting.
...
The seventh of April, 2020, has become the twentieth of April in the year 7. R read `07` as the year, `04` as the month and `2020` as the day, then rolled the impossible day number forward. There is no error and no warning, and a `filter()` built on that comparison would return all 345 rows while looking reasonable. Writing dates in ISO order avoids this.

One further point about `filter()`:
...
One equals sign is assignment, two is comparison. dplyr recognises the mistake and reports which one you meant. Base R would have treated `date` as the name of an argument instead, without an error.
...
`desc()` may look like clutter on a numeric column, but the obvious alternative does not work:
...
That column is the string `"Singapore"` in all 345 rows. It is there because the data hub serves many countries from one schema, and it carries no information once you have filtered down to one of them.

### `rename()` changes column names

`retail_and_recreation_percent_change_from_baseline` is fifty characters long. The name comes from Google's published series and is descriptive rather than convenient to type.
...
New name on the left, old name on the right. This matches `mutate()` and the other functions that create columns: the thing being created sits on the left of the `=`.
...
The top panel is the shape most dashboards showed in 2020. It rises steeply through the middle of the year and flattens at the right-hand edge, which is the only shape a cumulative count can take. The peak is not visible in it, and neither is the fact that half the year's cases had been confirmed by 20 May.
...
The two presentations also differ in what they allow to be claimed. A cumulative curve continues to rise after an intervention whatever the intervention did, so it can usually be described as flattening. An incidence curve makes the claim checkable: the number either came down or it did not.
...
That is what makes the error hard to catch. A bug that produces obvious nonsense is noticed at once; a bug that produces 337 plausible numbers and 8 impossible ones may not surface until a reviewer asks about a negative value in a figure.

Put `arrange(date)` in the chain immediately before the `mutate()`. It costs one line, and it does nothing when the data is already sorted by date, which this file is.
...
`lag()` on its own leaves `NA` at the top, because there is no previous row to subtract. The `default = 0` argument fills that gap with a zero instead. Whether that is appropriate depends on the data rather than on R.
...
`confirmed` is 0 on 22 January 2020. Zero cases had been confirmed by the end of that day, so zero cases were confirmed *during* it. That is an observation, not a gap. Recording it as `NA` would discard a genuine zero and shorten the series by one day in every count, sum and plot downstream.
...
The choice therefore depends on the series rather than on a rule to memorise. Look at the first value: if it is zero, `default = 0` records a true zero; if it is not, plain `lag()` leaves an `NA`, which is an accurate statement of what is not known. This file starts at zero, which is why `default = 0` is used in the code above, and that reason is worth writing in a comment beside it.
...
Zero rows. What comes back is an empty tibble rather than nothing at all, and the empty tibble still carries its column names and types, so you can see that the check ran rather than failed.
