...
That is what makes the error hard to catch. A bug that produces obvious nonsense is noticed at once; a bug that produces 337 plausible numbers and 8 impossible ones can survive until the negative values appear in a figure and have to be explained.
...
Zero rows. What comes back is an empty tibble, and it still carries its column names and types, so you can see that the check ran rather than failed.
